...
$sql = $connect->row("SELECT * FROM images WHERE user_id='$login_id' ORDER BY id_img DESC LIMIT 6 OFFSET $page");
if ($sql != NULL) {
    $i = 1;
    ...
    foreach ($sql as $data) {
        ...
        echo "<div class='image_view' id = ". $i." style='display: none;'>
                      ...
                      <form id='del_image' method='post'>
                          <input hidden name='img' value='".json_encode(array(
                'img_id' => $img_id,
                'img_src' => $image)) ."'>  
                          <input type='submit' value='Delete image' name='del_img'>
                      </form>
        ...
        echo "<button name='set_like' value='". json_encode($button)."'></button>
              <p>".$likes."</p>
           </div>
           <div id='comments'>
    ...
    echo "<p id='no_photos'>You have no photos yet.</p>";
}

$count_sql = $connect->row("SELECT COUNT(*) AS `count` FROM images WHERE user_id='$login_id'");
...
if (isset($_POST['del_img'])) {
    $button = json_decode($_POST['img'], true);
    $img_id = $button['img_id'];
    $img_src = $button['img_src'];
    unlink($img_src);
    $connect->query("DELETE FROM `images` WHERE `id_img` = '$img_id'");
    echo "<meta http-equiv='refresh' content='0'>";
}
